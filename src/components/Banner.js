...
export default function Banner() {
  ...
  const fetchData = async () => {
    ...
    const request = await axios.get(requests.fetchNowPlaying); //await을 해야 모든 데이터를 받고 변수에 대입함
    ...
    const { data : movieDetail } = await axios.get(`movie/${movieId}`, { //movieDetail안에 데이터들을 넣음
